...
HELEKET_CALLBACK_URL = os.getenv('HELEKET_CALLBACK_URL', '')  # устаревший, для обратной совместимости

# Домен и порт мини-приложения для формирования webhook URL: https://ДОМЕН:ПОРТ/heleket
_HELEKET_WEBHOOK_DOMAIN = os.getenv('HELEKET_WEBHOOK_DOMAIN', '')  # например: example.com
_HELEKET_WEBHOOK_PORT = os.getenv('HELEKET_WEBHOOK_PORT', '')       # например: 8443
# Либо можно указать URL целиком через HELEKET_WEBHOOK_URL (имеет приоритет)
_HELEKET_WEBHOOK_URL_OVERRIDE = os.getenv('HELEKET_WEBHOOK_URL', '')

def _build_webhook_url() -> str:
    """Собрать URL вебхука из переменных окружения"""
    if _HELEKET_WEBHOOK_URL_OVERRIDE:
        return _HELEKET_WEBHOOK_URL_OVERRIDE
    if _HELEKET_WEBHOOK_DOMAIN:
...
class HeleketAPI:
    ...
    def create_payment(self, amount: float, user_id: int, 
        ...
        # Криптовалюта и сеть — передаём только если явно указаны.
        # Если не переданы, Heleket покажет выбор валюты и сети прямо на странице оплаты.
        if to_currency and to_currency.strip():
            payload['to_currency'] = to_currency.strip()

        if network and network.strip():
            payload['network'] = network.strip()
        
        # URL вебхука: https://ДОМЕН:ПОРТ/heleket
        webhook_url = _build_webhook_url()
        if webhook_url:
            payload['url_callback'] = webhook_url
        else:
            logger.warning("Heleket: url_callback не задан. Установите HELEKET_WEBHOOK_URL или HELEKET_WEBHOOK_DOMAIN")
