...
function getExtraDevicePrice(plan: Plan): number {
  return PLAN_EXTRA_DEVICE_PRICE[plan.days] ?? 50;
}
...
const AnimatedBackground: React.FC<{ children: React.ReactNode; topPad?: number }> = ({ children, topPad = 0 }) => (
  <div className="max-w-md mx-auto min-h-screen w-full relative text-zinc-200 font-sans selection:bg-orange-500/30 tg-safe-padding overflow-x-clip" style={{ background: '#000', isolation: 'isolate', paddingTop: topPad > 0 ? `${topPad}px` : undefined }}>
    <style>{`
      ...
      /* ====== SETUP WIZARD (subscription → instruction) ====== */
      @keyframes setupRailFill {
      ...
      @keyframes setupSpark {
        0%   { transform: scale(0.6) rotate(-8deg); opacity: 0; }
        40%  { opacity: 1; }
        100% { transform: scale(1) rotate(0deg); opacity: 1; }
      }
      .setup-step { animation: contentStep 0.5s cubic-bezier(0.22,1,0.36,1) both; }
      .setup-icon-wrap {
        position: relative;
      ...
      .setup-progress-fill {
        ...
        animation: setupRailFill 0.45s cubic-bezier(0.22,1,0.36,1) both;
      }
      .setup-chip {
        transition: transform .2s ease, border-color .2s ease, background-color .2s ease, box-shadow .2s ease;
      }
      .setup-chip:active { transform: scale(0.96); }
      .setup-chip.active {
        border-color: rgba(249,115,22,0.55);
        background: rgba(234,88,12,0.14);
        box-shadow: 0 0 0 1px rgba(234,88,12,0.2), 0 8px 24px -12px rgba(234,88,12,0.45);
      }
      ...
      {wizardStep === 4 && (
        <div className="flex-1 flex flex-col h-full">
            ...
            <div className="mt-auto space-y-3">
              ...
              >
                Начать настройку <ArrowRight size={18} />
              </Button>
